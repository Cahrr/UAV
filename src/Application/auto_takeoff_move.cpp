...
/*    while(ros::ok()){
    ...
    while(i < sleep_time)
    {
        Command_now.command = Move_ENU;
        Command_now.sub_mode = 0;
        Command_now.pos_sp[0] = 1.0;
        Command_now.pos_sp[1] = 1.0;
        Command_now.pos_sp[2] = height_square;
        Command_now.yaw_sp = 0;
        Command_now.comid = comid;
        command_pub.publish(Command_now);
    ...
    /*while(ros::ok()){
        ros::spinOnce();
        if(detect_vel.flag == 1 || detect_vel.flag == 2){//判断能否检测到mark，由于飞行高度不同flag可以等于1或2
            break;
        }
